...
def estimate_width_3dr(img1_path, img2_path, predictor, cfg, backend="detectron2"):
    img1_rgb = read_rgbimg(img1_path)
    img2_rgb = read_rgbimg(img2_path)
    mask1, _, _, _ = analysis.segment_sidewalk_mask(img1_rgb, backend, predictor, cfg)
    mask2, _, _, _ = analysis.segment_sidewalk_mask(img2_rgb, backend, predictor, cfg)
